...
#include "trained_detector/facesMsg.h"
#include "trained_detector/faceMsg.h"
...
int main(int argc, char **argv)
{
  ...
  ros::NodeHandle nh;
  std::string path = ros::package::getPath("trained_detector");
  ...
  human_pub = nh.advertise<trained_detector::facesMsg>("human_detection_result", 1);
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    if( !frame.empty() ){
       ...
       trained_detector::facesMsg fsMsg;
       for(int i=0; i<faces.size(); i++){
	  trained_detector::faceMsg fMsg;
	  fMsg.x = faces[i].x;
	  fMsg.y = faces[i].y;
