...
#include "sleeping_detector/detectedobjectsMsg.h"
#include "sleeping_detector/objectMsg.h"
...
string path = ros::package::getPath("sleeping_detector");
...
int main(int argc, char **argv)
{
  ...
  humanPub = nh.advertise<sleeping_detector::detectedobjectsMsg>(humanPubTopicName, 1);
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
	...
	objs2 = non_max_suppression(objs2, nmsTresh);
       ...
       sleeping_detector::detectedobjectsMsg osMsg;
	osMsg.found = false;
	int aria=0;
	int target=0;
	if(objs1.size() > 0){
		osMsg.found = true;
		  ...
		  sleeping_detector::objectMsg oMsg;
		  oMsg.x = objs1[i].x;
		  oMsg.y = objs1[i].y;
